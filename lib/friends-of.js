...
function friendsOf(adjacencyList, name, distance) {
  if(Object.keys(adjacencyList).length === 0) return undefined;
  if(Object.keys(adjacencyList).length === 1) return adjacencyList[name];
  if( name in adjacencyList){
    let totFriends = new Set();
    friendsOfRecursion(name, adjacencyList, totFriends, distance, 0);

    totFriends.delete(name);

    return Array.from(totFriends);
  }
